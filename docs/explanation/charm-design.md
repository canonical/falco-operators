...
For example, the Falco operator observes the `http-endpoint` relation and triggers reconciliation when it changes, while the Falcosidekick k8s operator observes multiple relations including `send-loki-logs`, `certificates`, and `ingress`.
...
sequenceDiagram
    ...
    participant W as workload module
...
Both charms use Jinja2 templates for configuration files:

- Templates are stored in the `src/templates/` directory
- The workload module renders templates with the charm state as context
- Rendered files are installed in the appropriate locations
