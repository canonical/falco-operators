...
The following diagram shows how Falco is deployed as a subordinate charm alongside a principal charm:

```{mermaid}
graph LR
    subgraph "Physical/Virtual Machine"
        subgraph "Principal Charm"
            P[Principal Application]
        end
        subgraph "Falco Subordinate Charm"
            F[Falco Service]
        end
        P -.general-info relation.-> F
    end
...
1. Falco runs as a subordinate charm, deployed alongside a principal charm (such as the Kubernetes charm)
2. Falco monitors system calls and kernel events on the host machine
3. When suspicious activity is detected, Falco sends alerts to Falcosidekick using the `http-endpoint` relation
4. The relationship between Falco and the principal charm is established through the `general-info` relation

## Kubernetes charm deployment architecture
...
The charms support the following integration points:
